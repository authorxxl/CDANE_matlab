function EvaluateValue = ClusterEvaluateIndex(ClusterLabel, TrueLabel)
%% 评价聚类指标结果

%% 输入
% ClusterLabel = 1xn 的聚类结果标签矩阵
% TrueLabel    = 1xn 的真实结果标签矩阵

%% 输出
% EvaluateValue= 聚类结果评价指标值(Specificity, Rand Index, Sensitivity, NMI, Purity,ACC)

%% 代码

    EvaluateValue = zeros(1,6);  
    res = bestMap(TrueLabel, ClusterLabel);
    result = Evaluate(res, TrueLabel);
    EvaluateValue(1) = result(1);
    EvaluateValue(2) = result(2);
    EvaluateValue(3) = rand_index(res, TrueLabel);
    
    result = ClusteringMeasure(TrueLabel, ClusterLabel);
    EvaluateValue(4) = result(2);
    EvaluateValue(5) = result(3);
     EvaluateValue(6) = result(1);
    
    % EvaluateValue = [Specificity, Rand Index, Sensitivity, NMI, Purity];
end
...
%%
function EVAL = Evaluate(PREDICTED,ACTUAL)
...
% Output: EVAL = Row matrix with all the performance measures
    ...
    EVAL = [specificity sensitivity];
end

%%
function val = rand_index(idx,T)
...
%     idx =1xn matrix of the clustering results
    ...
    val=200*(rand_ss1+rand_dd1)/(n*(n-1))/100;
end